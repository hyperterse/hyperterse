...
func generateConnectorsFile(enum EnumInfo) error {
	...
	buf.WriteString("// Source: proto/connectors/connectors.proto\n\n")
	buf.WriteString("package types\n\n")
	buf.WriteString("import (\n")
	buf.WriteString("\t\"fmt\"\n\n")
	buf.WriteString("\t\"github.com/hyperterse/hyperterse/core/proto/connectors\"\n")
	buf.WriteString(")\n\n")
	...
	buf.WriteString("func StringToConnectorEnum(s string) (connectors.Connector, error) {\n")
	buf.WriteString("\tswitch s {\n")
	for _, val := range enum.Values {
		constName := toGoConstantName(val.StringVal, "Connector")
		protoConstName := fmt.Sprintf("connectors.Connector_%s", val.ProtoName)
		buf.WriteString(fmt.Sprintf("\tcase string(%s):\n", constName))
		buf.WriteString(fmt.Sprintf("\t\treturn %s, nil\n", protoConstName))
	}
	buf.WriteString("\tdefault:\n")
	buf.WriteString("\t\treturn connectors.Connector_CONNECTOR_UNSPECIFIED, fmt.Errorf(\"invalid connector type: %s\", s)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("}\n")
...
func generatePrimitivesFile(enum EnumInfo) error {
	...
	buf.WriteString("// Code generated by scripts/generate_types/script.go. DO NOT EDIT.\n")
	buf.WriteString("// Source: proto/primitives/primitives.proto\n\n")
	buf.WriteString("package types\n\n")
	buf.WriteString("import (\n")
	buf.WriteString("\t\"fmt\"\n\n")
	buf.WriteString("\t\"github.com/hyperterse/hyperterse/core/proto/primitives\"\n")
	buf.WriteString(")\n\n")
	...
	buf.WriteString("func StringToPrimitiveEnum(s string) (primitives.Primitive, error) {\n")
	buf.WriteString("\tswitch s {\n")
	for _, val := range enum.Values {
		constName := toGoConstantName(val.StringVal, "Primitive")
		protoConstName := fmt.Sprintf("primitives.Primitive_%s", val.ProtoName)
		buf.WriteString(fmt.Sprintf("\tcase string(%s):\n", constName))
		buf.WriteString(fmt.Sprintf("\t\treturn %s, nil\n", protoConstName))
	}
	buf.WriteString("\tdefault:\n")
	buf.WriteString("\t\treturn primitives.Primitive_PRIMITIVE_UNSPECIFIED, fmt.Errorf(\"invalid primitive type: %s\", s)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("}\n\n")

	// PrimitiveEnumToString function
	buf.WriteString("// PrimitiveEnumToString converts a protobuf Primitive enum to its string representation.\n")
	buf.WriteString("// e.g., PRIMITIVE_STRING -> \"string\", PRIMITIVE_INT -> \"int\"\n")
	buf.WriteString("func PrimitiveEnumToString(p primitives.Primitive) string {\n")
