...
<Note>
  Hyperterse is a long-running server process, not a serverless function. Deploy it on a container platform ([AWS](/deployment/aws), [GCP](/deployment/gcp), [Azure](/deployment/azure), [Railway](/deployment/railway), [DigitalOcean](/deployment/digital-ocean)) or [bare metal](/deployment/bare-metal), then put Cloudflare in front of it.
</Note>
