...
When run from the project root, the CLI loads `.hyperterse` automatically. Use a positional argument to point to a different project directory: `hyperterse build path/to/project -o dist`.
...
|---|---|---|---|
| `--file` | `-f` | `""` | Path to config file. Alternative to positional arg. Default: `.hyperterse` in current directory. |
...
If `dist/` contains `model.bin`, passing the directory is sufficient. Alternatively, pass the manifest file directly as the positional argument: `hyperterse serve dist/model.bin`.
...
|---|---|---|
| `--file` | `-f` | Path to `model.bin`. Alternative to positional arg. Default: `./model.bin`. |
| `--port` | `-p` | Server port (overrides manifest and `PORT` env var). |
| `--log-level` | | Log level: 1=ERROR, 2=WARN, 3=INFO, 4=DEBUG (overrides manifest). |
...
```
┌──────────────────────────────────────────────────┐
│  1. hyperterse validate                           │
│  2. hyperterse build -o dist                      │
...
jobs:
  build:
    ...
    steps:
      ...
      - run: ./dist/hyperterse validate
      - run: ./dist/hyperterse build -o dist
