...
This loads `.hyperterse` from the current directory, discovers all adapters and routes, bundles scripts, and writes the deployment artifact.
...
| `model.bin` | Serialized manifest (adapters, routes, config) |
| `build/vendor.js` | Shared dependency bundle (if routes import npm packages) |
...
<Frame caption="Validate once, build once, deploy the artifact">
  <img src="/assets/diagrams/deployment-workflow.svg" alt="Deployment workflow: validate, build, test locally, ship artifact, serve" />
</Frame>
