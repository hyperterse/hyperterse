...
The `.hyperterse` file contains service-level settings: name, version, server port, log level, cache defaults, build options, and discovery defaults for adapters/tools/agents. It does not contain adapter, tool, or agent definitions — those live in `app/`.
