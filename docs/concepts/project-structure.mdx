...
The compiler scans `app/agents/*/config.terse` at build time. Each `config.terse` file defines one ADK-backed runtime agent. The directory name becomes the agent name unless explicitly overridden by `name`.
