...
**Hybrid tools** combine a DB adapter with script transforms for pre-processing and post-processing. Execution order is: auth, input transform, DB execution, output transform.
...
See [Tool configuration reference](/reference/tool-config) for the complete field specification, including all input properties, output definitions, and cache overrides.
