...
Each `app/tools/*/config.terse` defines one MCP tool — its execution strategy, inputs, outputs, scripts, authentication, and caching behavior.
...
<ParamField body="use" type="string | string[]">
  ...
  <Warning>
  Required if `scripts.handler` is not set. A tool must have either `use` or `scripts.handler` (or both).
  </Warning>
</ParamField>
...
<ParamField body="scripts" type="object">
  Script paths for custom logic. All paths are relative to the tool directory.
...
<ParamField body="cache" type="object">
  ...
  <Expandable title="properties">
    <ParamField body="enabled" type="boolean">
      Whether caching is active for this tool. Inherits from `server.queries.cache.enabled` if not set.
    </ParamField>

    <ParamField body="ttl" type="integer">
      Cache TTL in seconds for this tool. Inherits from `server.queries.cache.ttl` if not set.
    </ParamField>
...
2. `scripts.handler` is defined (script-backed tool).

<Warning>
  Tools with neither `use` nor `scripts.handler` are rejected during
  validation.
</Warning>
