...
MongoDB is a document-oriented NoSQL database. Hyperterse provides support for MongoDB using the official [MongoDB Go Driver](https://github.com/mongodb/mongo-go-driver), enabling you to run find, findOne, insertOne, insertMany, updateOne, updateMany, deleteOne, deleteMany, aggregate, and countDocuments operations.
...
For a complete list of connection options, see the [MongoDB Go Driver connection documentation](https://www.mongodb.com/docs/drivers/go/current/fundamentals/connection/).
...
MongoDB queries in Hyperterse use a **JSON statement**. The `statement` field is a JSON object with `database`, `collection`, `operation`, and operation-specific fields. Use `{{ inputs.name }}` for parameterized values; the executor substitutes these before the connector runs the operation.
...
queries:
  get-user:
    ...
    statement: |
      {"database":"mydb","collection":"users","operation":"findOne","filter":{"_id":"{{ inputs.userId }}"}}
    inputs:
      userId:
        type: string
```
...
queries:
  update-user:
    ...
    statement: |
      {"database":"mydb","collection":"users","operation":"updateOne","filter":{"_id":"{{ inputs.userId }}"},"update":{"$set":{"email":"{{ inputs.email }}"}}}
    inputs:
      userId:
        type: string
      email:
...
queries:
  delete-user:
    ...
    statement: |
      {"database":"mydb","collection":"users","operation":"deleteOne","filter":{"_id":"{{ inputs.userId }}"}}
    inputs:
      userId:
        type: string
```
