...
| Field | Type   | Description                                                      |
| ----- | ------ | ---------------------------------------------------------------- |
| `out` | string | Output directory path (script filename uses config `name` field) |

**Rules:**

- The script filename is always derived from the config `name` field
- CLI flag (`--out/-o`) takes precedence over config settings
- Default is `dist` directory if not specified
