...
This guide walks through building a complete Hyperterse configuration for a typical application—a user management system with authentication and analytics.

## Configuration formats

Hyperterse supports two configuration formats. Choose whichever you prefer:

<Tabs>
  <TabItem label="YAML (.terse)">
...
````
  ...
  <TabItem label="Dsl (.Hyperterse)">
```text title="config.hyperterse"
adapter main_db {
  connector: postgres
  connection_string: "postgresql://user:pass@localhost:5432/app"
}
